...
void string2biset(vector<Block> &v,const string &str)
{
    string temp;
    for(int k=0;k<str.size();k+=8)
    {
        if(k+8>str.size())
...
string biset2hex(const Block &b)
{
    bitset<32> b1,b2;
    for(int i=0;i<32;i++)
        b1[i]=b[i];
    ...
    strstream1<<hex<<uppercase<<b1.to_ulong();
    strstream2<<hex<<uppercase<<b2.to_ulong();
    return strstream2.str()+strstream1.str();
}

void Hex2Biset(vector<Block> &v,string str)
{
    std::vector<HBlock> v1;
    string tmp;
    unsigned long a;
    ...
    for(int i=0;i<v1.size();i+=2)
    {
        ...
        {
            tmp1=v1[i].to_string()+v1[i+1].to_string();
        }
        Block Bb(tmp1);
        v.push_back(Bb);
    }
